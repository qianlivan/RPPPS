...
# version 20190608
# Usage: 
#       python fitsio_splitpol_FASTpsrfits.py fileroot filename
# Example: 
#       python fitsio_splitpol_FASTpsrfits.py output file.fits
#------------------------------------------------------------------
...
npol=1
data2 = np.zeros(nline,dtype=[('TSUBINT','float64'),('OFFS_SUB','float64'),('LST_SUB','float64'),('RA_SUB','float64'),('DEC_SUB','float64'),('GLON_SUB','float64'),('GLAT_SUB','float64'),('FD_ANG','float32'),('POS_ANG','float32'),('PAR_ANG','float32'),('TEL_AZ','float32'),('TEL_ZEN','float32'),('DAT_FREQ','float32',(nchan)),('DAT_WTS','float32',(nchan)),('DAT_OFFS','float32',(nchan)),('DAT_SCL','float32',(nchan)),('DATA','uint8',(nsblk,npol,nchan,1))])
...
data=fits[1].read()
#data2['DATA'][:][:,0,:,0] = (data['DATA'][:][:,0,:,0]+data['DATA'][:][:,1,:,0])/2.0
data2['TSUBINT'][:] = data['TSUBINT'][:]
data2['OFFS_SUB'][:] = data['OFFS_SUB'][:]
data2['LST_SUB'][:] = data['LST_SUB'][:]
data2['RA_SUB'][:] = data['RA_SUB'][:]
data2['DEC_SUB'][:] = data['DEC_SUB'][:]
data2['GLON_SUB'][:] = data['GLON_SUB'][:]
data2['GLAT_SUB'][:] = data['GLAT_SUB'][:]
data2['FD_ANG'][:] = data['FD_ANG'][:]
data2['POS_ANG'][:]= data['POS_ANG'][:]
data2['PAR_ANG'][:] = data['PAR_ANG'][:]
data2['TEL_AZ'][:] = data['TEL_AZ'][:]
data2['TEL_ZEN'][:] = data['TEL_ZEN'][:]
data2['DAT_FREQ'][:] = data['DAT_FREQ'][:]
#data2['DAT_WTS'][:] = data['DAT_WTS'][:][0:nchan]

#data2['DAT_OFFS'][:][:] = data['DAT_OFFS'][:][0:nchan]
#data2['DAT_SCL'][:][:] = data['DAT_SCL'][:][:][0:nchan]

for rowindex in range(nline):
#    data=fits[1].read(rows=[rowindex], columns=['DATA'])
#    data2['DATA'][rowindex][:,0,:,0]=(data[0][0][:,0,:,0]+data[0][0][:,1,:,0])/2.0
    data2['DATA'][rowindex][:,0,:,0] = (data['DATA'][rowindex][:,0,:,0]+data['DATA'][rowindex][:,1,:,0])/2.0
    data2['DAT_WTS'][rowindex][:] = data['DAT_WTS'][rowindex][0:nchan]

    data2['DAT_OFFS'][rowindex][:] = data['DAT_OFFS'][rowindex][0:nchan]
    data2['DAT_SCL'][rowindex][:] = data['DAT_SCL'][rowindex][0:nchan]
